...
{TINYMCE}
    ...
    var savekat = -1;
    function filesearch() {
        f=open("dokument.php?P=1","File","width=900,height=650,left=200,top=100, scrollbars=yes");
        f.pickup = true;
    }
    ...
    function editContent( editor ) {
        ...
        $.get( 'jqhelp/wissen.php?task=edit&id='+id+'&edit='+editor+'&tiny='+tiny, function(content) {
                $('#wissencontent').empty().append(content);
        });
    }
    function newContent() { 
        $.get('jqhelp/wissen.php?task=neu&tiny='+tiny, function( content ) {
            ...
            success: function(rc) {
                if ( rc.cnt == 0 ) {
                    if ( wort != '' ) {
                        ...
                        $('#wissencontent').empty();
                        showSel();
                        $('#headline').empty().append('[<b>'+$('#catname').text()+'</b>]');
                    }
                } else {
                   if ( rc.cnt  == 1 ) {
                       showcontent( rc );
    ...
    function openpath( hg ) {
        var subhg = $('#C'+hg).attr('class');
        path.push(subhg);
        while ( subhg != 'sub0' ) {
            subhg = $('#C'+subhg.substr(3)).attr('class');
            path.push(subhg);
        }
        while ( path.length > 0 ) {
            x = path.pop();
        ...
        toggleMenu($('#C'+hg).attr('class').substr(3),hg);
    }
    function saveCat() {
            var newcat = $('#catneu').val();
            var hg = $('#hg').val();
            var cid = $('#kat').val()
            var kdhelp = ( $('#kdhelp').is(':checked') )?'t':'f';
    ...
    function toggleMenu( hg, id ) {
        ...
        $('#hg').val(hg);
        $('#wort').val('');
        $('#catname').empty().append($('#C'+id).text());
        $('#editcat').show();
        suche(); 
        if ( $('.sub'+id).is(':hidden') ) {
...
<h1 class="toplist  ui-widget  ui-corner-all tools content1" onClick="help('WissensDB');">.:knowhowdb:.</h1><br>
...
<span style="position:absolute; left:1em; top:7em; width:95%; border: 0px solid black">
	<div style="float:left; width:33%; text-align:left; border: 0px solid red" >
        <strong onClick='mkMenu(-1);'>.:categories:.</strong><br>
...
<!-- END OwenerListe -->
    ...
    <button onClick="self.close();" style='visibility:{close};' >.:close:.</button>
</span>
</div>
{END_CONTENT}
</body>
