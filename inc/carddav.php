...
class carddav_backend
{
	...
	*/
	...
	/**
     * zusätzlich erlaubter Returncode
	 * @var	integer
	 */
    private $rc_code = -1;
	 ...
	 */
	public function __construct($url = null, $errorcode = null)
	{
		if ($url !== null)
		{
			$this->set_url($url);
	...
	*/
	...
	public function set_error($code) {
        if ( (int)$code > 100 && (int)$code < 900 ) $this->rc_code = (int)$code;
    }
	...
	*/
	...
	public function add($vcard, $vcard_id = null)
	{
		...
		switch($result['http_code'])
		{
			...
			default:
                if ( $vcard_id === null) {
				    throw new Exception('http status code ' . $result['http_code'] . '.', 
                                        self::EXCEPTION_WRONG_HTTP_STATUS_CODE_ADD);
                } else {
				    throw new Exception('http status code ' . $result['http_code'] . '.', 
                                        self::EXCEPTION_WRONG_HTTP_STATUS_CODE_UPDATE);
                };
			break;
		}
	}
