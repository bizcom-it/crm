...
include($_SESSION['erppath'].'crm/inc/conf.php');
include($_SESSION['erppath'].'crm/inc/version.php');
if ( ob_get_level() == 0 ) ob_start();
echo "<br>Installation der Version $VERSION";
echo " der Datenbankinstanz: ".$_SESSION["dbname"]."<br>";
ob_flush();
flush();
if ( !file_exists($_SESSION['erppath'].'crm/log') ) mkdir($_SESSION['erppath'].'crm/log',0755);
if ( $log = @fopen($_SESSION['erppath'].'crm/log/install.log',"a") ) {
	$logfile = $_SESSION['erppath']."crm/log/install.log";
...
foreach ( $mkdir as $chk ) {
    if ( !file_exists($_SESSION['erppath'].'crm/'.$chk) )  {
        $rc = mkdir($_SESSION['erppath'].'crm/'.$chk, $_SESSION['dir_mode']);
        if ( $rc ) { 
            if ( $_SESSION["dir_group"] ) chgrp($_SESSION['erppath'].'crm/'.$chk, $_SESSION['dir_group']);
...
foreach ( $writeable as $chk ) {
    if ( is_writable($_SESSION['erppath'].'crm/'.$chk) ) {
	    echo "$chk : $ok<br>"; 
        fputs($log,"$chk : ok\n");
    } else {
...
//ERP da?
	$OK = is_file($_SESSION['erppath']."config/$erpConfigFile.conf");
	fputs($log,"$ERPNAME : ");
...
$f = fopen($_SESSION['erppath']."sql/Pg-upgrade2/crm.sql","r");
if ( !$f ) { 
	echo "Kann Datei crm.sql nicht &ouml;ffnen.";
...
while ( !feof($f) ) {
	...
	if ( !preg_match("/;$/",$zeile) ) { 
		...
		$query .= $zeile;
		$rc = $GLOBALS['db']->query(substr($query,0,-1));
		if ( $rc ) { $OK++; echo ".";}
		else { 
			$fehl++; 
			echo "!"; 
