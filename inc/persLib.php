...
function getKontaktStamm($id,$pfad='') {
    ...
    if( !$rs ) {
        ...
        $daten['cp_notes'] = strtr($daten['cp_notes'],array("\r"=>'<br>'));
        $v = glob("$pfad./dokumente/".$_SESSION['dbname']."/$tab$cnr$id/vcard$id.*");
        if ( count($v) > 0 ) { $daten['visitenkarte'] = $v[0]; } 
        else {  $daten['visitenkarte'] = ''; };
        $daten['Firma']        = $firma;
...
function getAllPerson($sw, $usePre=true) {
    ...
    if ( $sw[0] === 0 ) { 
        $where = "cp_phone1 like '$Pre".$sw[1]."%' or cp_mobile1 like '$Pre".$sw[1]."%' "; 
    } else if ( $sw[0] == 2 ) { 
         $where = "cp_zipcode like '".$sw[1]."%' ";
    } else  { 
         $where = "cp_name ilike '$Pre".$sw[1]."%' or cp_givenname ilike '$Pre".$sw[1]."%'"; 
    }
...
function suchPerson($muster) {
    ...
    $fuzzy  = $muster['fuzzy'];
    $andor  = $muster['andor'];
    $rechte = berechtigung('P.cp_');
    $joinCustomer = ' left join customer C on P.cp_cv_id=C.id';
    $joinVendor   = ' left join vendor V on P.cp_cv_id=V.id';
    ...
    if ( $muster['cp_name'] == '~' ) {
        ...
        if ( isset( $muster['customer_name'] ) ) {
            $whereCustomer  = "C.name ilike '$pre" . $muster['customer_name'] . "$fuzzy'";  
    ...
    $felderContact                 = 'P.cp_id, P.cp_cv_id, P.cp_title, P.cp_name, P.cp_givenname, P.cp_fax, P.cp_email, P.cp_gender as cp_gender';
    $felderContcatOrCustomerVendor = "COALESCE (P.cp_country, country) as cp_country,COALESCE (P.cp_zipcode, zipcode) as cp_zipcode, 
                                      COALESCE (P.cp_city, city) as cp_city, COALESCE (P.cp_street, street) as cp_street, 
                                      COALESCE (NULLIF (P.cp_phone1, ''), NULLIF (P.cp_mobile1, ''), phone) as cp_phone1";
    ...
    if ( $muster['customer'] ){     //auf checkbox customer mit Titel Kunden prüfen
        $sql0 = "select $felderContact, $felderContcatOrCustomerVendor, C.name as name, C.language_id as language_id, 'C' as tbl 
                 from contacts P$joinCustomer where P.cp_cv_id=C.id and ($whereCustomer $andor $where) and $rechte order by cp_name";       
        $rs0 = $GLOBALS['db']->getAll($sql0);
        if (!$rs0) $rs0 = array();
    }
    $rs1 = array(); //s.o.
    if ( $muster['vendor'] ){ //auf checkbox vendor mit Titel Lieferant prüfen
        $sql0 = "select $felderContact, $felderContcatOrCustomerVendor, V.name as name, V.language_id as language_id, 'V' as tbl 
                 from contacts P$joinVendor where P.cp_cv_id=V.id and ($whereVendor $andor $where) and $rechte order by cp_name";
        $rs1 = $GLOBALS['db']->getAll($sql0);
        if (!$rs1) $rs1 = array();
    }
    $rs2 = array(); //s.o.
    if ( isset( $muster['deleted'] ) ) { //auf checkbox deleted mit Titel "gelöschte Ansprechpartner (Kunden und Lieferanten)" prüfen
        ...
        $sql0 = "select $felderContact, P.cp_country, P.cp_zipcode, P.cp_city, P.cp_street, P.cp_phone1, '' as name,'P' as tbl 
                 from contacts P where $rechte and (".$where.") and P.cp_cv_id is null order by P.cp_name";
        $rs2 = $GLOBALS['db']->getAll($sql0);
        if (!$rs2) $rs2 = array();
    }
...
function savePersonStamm($daten,$datei) {
    ...
    if ( $fehler==-1 ) { //Kein Fehler aufgetreten
        ...
        if ( $GLOBALS['db']->query($sql0) ) {  //Erfolgreich gesichert
            return $pid;            
        } else {
            return 'unbekannt';
        }
...
function leertplP (&$t,$fid,$msg,$tab,$suche=false,$Quelle,$ui=false) {
        ...
        doBlock($t,'pers1','briefanred','BA',$anreden,'cp_salutation','cp_salutation','1'); 
        
        /*** Wird doch im leerem Template gar nicht verwendet... ***/
        /** Doch! Bei der Neueingabe will ich Owner und vorhandene Briefanreden schon haben **/
}
...
function vartplP (&$t,$daten,$msg,$btn1,$btn2,$btn3,$fld,$bgcol,$fid,$tab,$ui=false) {
        ...
        $Image  = '';
        $VCARD  = '';
        if ( (isset($daten['cp_cv_id']) && $daten['cp_cv_id']) && 
             (isset($daten['tbl']) && $daten['tbl']) ) {
            $fa     = getFirmenstamm($daten['cp_cv_id'],false,$daten['tbl']);
            $nummer = ($daten['tbl']=='C')?$fa['customernumber']:$fa['vendornumber'];
        }
        if ($nummer) {
            $root = 'dokumente/'.$_SESSION['dbname'].'/'.$daten['tbl'].$nummer.'/'.$daten['cp_id'];
        } else {
            $root = 'dokumente/'.$_SESSION['dbname'].'/'.$daten['cp_id'];
        };
        if ( isset($daten['cp_grafik']) && trim($daten['cp_grafik'])<>'') 
        ...
        if ( $tmp )  foreach ($tmp as $vcard) {
            ...
            if ( in_array($ext,array('jpg','jpeg','gif','png','pdf','ps')) ) {
                $VCARD = "<img src='$vcard' width='110' height='80'>";
                break;
            }
        }
