...
function mail_list($connection,$message="") {
    ...
    foreach ($response as $msg) {
        ...
        $msg->orgdate = sprintf("%04d%02d%02d%02d%02d",$date["year"],$date["month"],$date["day"],$date["hour"],$date["minute"]);
...
function mail_fetchstructure($conn,$mail) {
    return @imap_fetchstructure ($conn,$mail);
}

function mail_dele($conn,$mail) {
    $rc = @imap_delete($conn,$mail);
    $rc = mail_expunge($conn);
    return $rc;
}
...
function mail_flag($conn,$mail,$flag) {
    if ( substr($flag,0,2) != '\\' ) $flag = '\\'.$flag;
    return @imap_setflag_full($conn,$mail,$flag); 
}
function mail_unflag($conn,$mail,$flag) {
    if ( substr($flag,0,2) != '\\' ) $flag = '\\'.$flag;
    return @imap_clearflag_full($conn,$mail,$flag); 
}

function mail_answered($conn,$mail) {
    return @imap_setflag_full($conn,$mail,"\\Answered");
}

function mail_SetFlag($conn,$mail,$flag) {
    switch ( $flag ) {
        case 'Seen'      : return @imap_setflag_full($conn,$mail,'\\Seen'); 
                           break;
        case 'Unseen'    : return @imap_clearflag_full($conn,$mail,'\\Seen'); 
                           break;
        case 'Flagged'   : return @imap_setflag_full($conn,$mail,'\\Flagged'); 
                           break;
        case 'Unflagged' : return @imap_clearflag_full($conn,$mail,'\\Flagged'); 
                           break;
        case 'Deleted'   : mail_dele($conn,$mail);
                           break;
        case 'Draft'     : return @imap_setflag_full($conn,$mail,'\\Draft');
                           break;
        case 'Answerd'   : return @imap_setflag_full($conn,$mail,'\\Answered');
                           break;
...
function add_part_to_array($obj, $partno, & $part_array) {
    ...
    if ( $obj->type == 2 or $obj->type == 3 ) { // Check to see if the part is an attached email message, as in the RFC-822 type
...
function mail_get_file($conn,$mail,$part) {
    ...
    $f = @fopen($_SESSION['erppath']."crm/tmp/".$filename,"w");
	@fwrite($f,$file);
	@fclose($f);
