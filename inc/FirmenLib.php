...
*****************************************************/
function getShipStamm($id,$tab='C',$complete=false,$fid=0) {
    if ($complete) {
        $sql  = 'SELECT COALESCE(trans_id,id) as trans_id,COALESCE(shipto_id,id) as shipto_id,';
        $sql .= 'COALESCE(shiptoname,name) as shiptoname,';
        $sql .= 'COALESCE(shiptostreet,street) as shiptostreet,';
        $sql .= 'COALESCE(shiptocity,city) as shiptocity,';
        ...
        $sql .= 'COALESCE(shiptoemail,email) as shiptoemail,';
        $sql .= 'COALESCE(shiptocontact,contact) as shiptocontact FROM ';
        if ( $id > 0 ) {
            $sql .= 'shipto LEFT JOIN '.(($tab=='C')?'customer':'vendor').' ON trans_id=id WHERE shipto_id='.$id;
        } else {
            $sql .= (($tab=='C')?'customer':'vendor').' LEFT JOIN shipto ON trans_id=id WHERE id='.$fid;
        }
    } else {
...
function getAllFirmenByMail($sw,$usePre=true,$tab='C') {
    ...
    if ($tab=='C') {
        $sql="SELECT name,email,'C' as tab,id FROM customer WHERE ($where) and $rechte ORDER BY name";
    } else if ($tab=='V') {
        $sql="SELECT name,email,'V' as tab,id FROM vendor WHERE ($where) and $rechte ORDER BY name";
    } else if ($tab=='P') {
        $rechte = str_replace('owener','cp_owener',$rechte);
        $sql="SELECT cp_name||', '||cp_givenname as name,cp_email email,'P' as tab,cp_id as id FROM contacts WHERE (cp_$where) and $rechte ORDER BY cp_name";
    } else {
        return false;
    }
    $rs=$GLOBALS['db']->getAll($sql);
...
function getFirmenStamm($id,$ws=true,$tab='C',$cvar=true) {
    if ( $tab=='C' ) {
        ...
        $rs   = $GLOBALS['db']->getOne($sql);
        $op   = $rs['sum'];
        $sql  = "SELECT sum(amount) FROM ar WHERE customer_id=$id and transdate > now() - interval '1 year'";
        $rs   = $GLOBALS['db']->getOne($sql);
        $ums  = $rs['sum'];
        ...
        $rs   = $GLOBALS['db']->getOne($sql);
        $oa   = $rs['sum'];
        $sql  = "SELECT sum(amount) FROM ap WHERE vendor_id=$id and transdate > now() - interval '1 year'";
        $rs   = $GLOBALS['db']->getOne($sql);
        $ums  = $rs['sum'];
        $sql  = "SELECT C.*,E.name as verkaeufer,EMP.name as bearbeiter,B.description as kdtyp,B.discount as typrabatt,BL.bundesland,";
        $sql .= "L.lead as leadname,LA.description as language,'' as pricegroup,'' as terms_netto FROM vendor C LEFT JOIN employee EMP ON C.employee=EMP.id ";
    ...
    $row = $GLOBALS['db']->getOne($sql);  // Rechnungsanschrift
    if( !$row ) {
        return false;
    } else {
        ...
        if ($row['mtime']=='') {
            ...
            $rs2 = $GLOBALS['db']->getOne($sql);  // Rechnungsanschrift
        ...
        if ( $row['konzern'] ) {
            ...
            if ( $tab=='C' ) {
                $krs = $GLOBALS['db']->getOne(sprintf($sql,'customer',$row['konzern']));
            } else {
                $krs = $GLOBALS['db']->getOne(sprintf($sql,'vendor',$row['konzern']));
            }
    ...
    $daten['oa']        = $oa;
    $daten['nummer']    = $nummer;
    if ( !isset($daten['sync']) ) $daten['sync'] = 0;
...
function getCvarName($id) {
    ...
    $rs  = $GLOBALS['db']->getOne(sprintf($sql,$id,$id));
    return $rs['name'];
}
/****************************************************
* getAllShipto
...
function suchFirma($muster,$tab='C') {
    ...
    if ($muster['umsatz']) {
        ...
        if ($treffer[0]) {
            if ($tab=='C') {
                $umstpl = "(SELECT sum(amount) FROM ar WHERE customer_id=C.id $year)";
            } else {
                $umstpl = "(SELECT sum(amount) FROM ap WHERE vendor_id=V.id $year)";
            }
            foreach ($treffer[0] as $val) {
                if ($val[0] == '>' || $val[0] == '<' || $val[0] == '=') {
    ...
    if ($where<>'') {
        if ( $umsatz != '' ) {
            $sql="SELECT $cols,$umstpl as umsatz,'$tab' as tab FROM $tabs WHERE ($where $umsatz) and $rechte";
        } else {
            $sql="SELECT $cols,'$tab' as tab FROM $tabs WHERE ($where) and $rechte";
        }
        $rs=$GLOBALS['db']->getAll($sql);
        if(!$rs) {
            $daten=array();
...
function saveFirmaStamm($daten,$datei,$typ='C',$neu=false) {
    ...
    $dbfld=array(    'name' => array(0,1,1,'Name',75),        'greeting' => array(0,0,1,'Anrede',75),
        ...
        'sync' => array(0,0,6,'Synchronisation',0));
    ...
    if ( $fehler=='ok' ) {
        ...
        //include("links.php"); // Wozu?
        if (!is_dir($dir_abs.'/'.$DIR)) { // Firmenverzeichnis anlegen
            $rc = chkdir($DIR);  
        }
        if ( $tmpdata['sep_cust_vendor'] ) { 
            $link_dir_cv = ( $typ=='C' )?'/link_dir_cust':'/link_dir_vend';
        } else {
            $link_dir_cv = '/link_dir';
        };
        if (!is_dir($dir_abs.$link_dir_cv)) {
            $rc = chkdir($link_dir_cv);  
        }
        $linkname = mkDirName($daten['name']).'_'.$DIR;
        if ( !is_link($dir_abs.$link_dir_cv.'/'.$linkname) ) { // Noch kein Link erstellt
            symlink($dir_abs.'/'.$DIR, $dir_abs.$link_dir_cv.'/'.$linkname);
        } else {
            if ( readlink($dir_abs.$link_dir_cv.'/'.$linkname) != $DIR ) {  // Zeigt der Link auf das richtige Verzeichnis?
                unlink($dir_abs.$link_dir_cv.'/'.$linkname);
                symlink($dir_abs.'/'.$DIR, $dir_abs.$link_dir_cv.'/'.$linkname);
            }
        };
...
function newnr($typ,$bid=0) {
    $rc = $GLOBALS['db']->query('BEGIN');
    if ( $bid>0 ) {
        $rs = $GLOBALS['db']->getOne('SELECT customernumberinit  as '.$typ.'number FROM business WHERE id = '.$bid);
    } else {
        $rs = $GLOBALS['db']->getOne('SELECT '.$typ.'number FROM defaults');
    };
    ...
    $newnr = $pre.$y;
    if ( $bid>0 ) {
        $rc = $GLOBALS['db']->query("UPDATE business set customernumberinit='$newnr' WHERE id = $bid");
    } else {
        $rc = $GLOBALS['db']->query("UPDATE defaults set ".$typ."number='$newnr'");
    }
    if ( $rc ) { $GLOBALS['db']->query('COMMIT'); }
    else       { $GLOBALS['db']->query('ROLLBACK'); $newnr=''; };
    return $newnr;
}
...
function mknewFirma($id,$typ) {
   ...
   // $rc=$GLOBALS['db']->query($sql); Kommentiert bis ERP-Bug #2201 gefixt ist
...
function doReport($data,$typ='C') {
    ...
    if (!preg_match('/P./',$felder)) {
        ...
        if (preg_match('/shipto/i',$tabs) or preg_match('/S./',$felder)) {
            ...
            $sql="SELECT $felder FROM  ".$tab[$typ]." ".$typ." LEFT JOIN contacts P ";
...
function getTopParts($cid) {
    ...
    $ums   = $GLOBALS['db']->getAll($sql.'summe desc limit '.$limit);
    $qty   = $GLOBALS['db']->getAll($sql.'qty desc,ar.transdate desc limit '.$limit);
...
function leertpl (&$t,$tpl,$typ,$msg='',$suchmaske=false,$ui=false) {
        $tmpdata = getUserEmployee(array('GEODB','BLZDB','preon','searchtable'));
        if ( $tpl == 3 ) {
            $t->set_file(array('fa1' => 'firmen3.tpl'));
        } else {
            $t->set_file(array('fa1' => 'companies'.$tpl.'.tpl'));
        }
             if ( $typ == 'C' ) { $was = '.:Customer:.'; }
        else if ( $typ == 'V' ) { $was = '.:Vendor:.'; }
        else {                    $was = '.:Company:.'; }
        $menu =  $_SESSION['menu'];
        $t->set_var(array(
            'Q'             => $typ,
            ...
            'preon'         => ($tmpdata['preon'])?'checked':'',
            'tabelle'.$tmpdata['searchtable']  => 'checked',
            'extra1'        => ($tmpdata['searchtable'] == 'C')?'display':'none',
            'extra2'        => ($tmpdata['searchtable'] == 'V')?'display':'none',
            'terms'         => '',
            'kreditlim'     => '',
            'op'            => '',
            'preisgrp'      => '',
            ...
            'shiptoemail'    => '',
            'shiptocontact'  => '',
            'GEODB'          => ($tmpdata['GEODB']=='t')?'1==1':'1>2',
            //'GEOS'           => ($_SESSION['GEODB']=='t')?'visible':'hidden',
            'showGeo'        => ($tmpdata['GEODB']=='t')?'':'style="display:none"',
            'GEO1'           => ($tmpdata['GEODB']=='t')?'':'!--',
            'GEO2'           => ($tmpdata['GEODB']=='t')?'':'--',
            'BLZ1'           => ($tmpdata['BLZDB']=='t')?'':'!--',
            'BLZ2'           => ($tmpdata['BLZDB']=='t')?'':'--',
        ...
        $defcurr = $GLOBALS['db']->getOne('SELECT currency_id FROM defaults');
        $curr=getCurrencies();
        doBlock($t,'fa1','currency','C',$curr,'id','name',$defcurr['currency_id']); 
        if (!$suchmaske) {
...
function vartpl( &$t, $daten, $typ, $msg, $btn1, $btn2, $tpl, $suchmaske=false, $ui=false ) {
        ...
        $t->set_var(array(
                ...
                'Btn2'          => $btn2,
                'Msg'           => $msg,
                'preon'         => ( isset($daten['preon']) )            ? 'checked':'',
                'tabelle'.$typ  => 'checked',
                'action'        => $ui?'':'firmen'.$tpl.'.php',
                ...
                'GEODB'         => ($tmpdata['GEODB']=='t')?'1==1':'1>2',
                'GEOS'          => ($tmpdata['GEODB']=='t')?'visible':'hidden',
                'GEO1'          => ($tmpdata['GEODB']=='t')?'':'!--',
                'GEO2'          => ($tmpdata['GEODB']=='t')?'':'--',
                'BLZ1'          => ($tmpdata['BLZDB']=='t')?'':'!--',
                'BLZ2'          => ($tmpdata['BLZDB']=='t')?'':'--',
                'cvars'         => ( isset($daten['id']) )?cvar_edit($daten['id']):'', 
                'sync'.$daten['sync'] => 'checked',
                'variablen'     => $varablen
        ));
        $jahre = getUmsatzJahre(($typ=='C')?'ar':'ap');
        if ( !isset($daten['year']) ) $daten['year'] = date('Y');
