...
*/
 ...
 var $debug = false;
	...
	function document($id=false,$fname="",$fpath="",$descript="") {
		if ($this->debug) $this->f = fopen($_SESSION['erppath'].'crm/tmp/doc.log',"w");
		$this->log("newDoc");
		$this->db=$GLOBALS['db'];
		if ($id>0) {
...
*/
	function saveDocument() {
		...
		if (!$this->id) {
			...
			$werte=array('filename'=>$this->name,'descript'=>$this->descript,
                         'datum'=>date("Y-m-d"),'zeit'=>date("H:i:s"),'employee'=>$_SESSION["loginCRM"],
                         'pfad'=>$this->pfad,'lock'=>$this->lock);
		        $this->log("update");
		        $this->logvar();
			if ($this->size) {
				//wird bei einem Upload gesetzt.
				$felder[]='size';
				$werte['size']=$this->size;
			}
...
//$up=fopen("/tmp/uoDo.log","a");
//fputs($up,"$pfad\n");
//fputs($up,print_r($file,true));
		...
		$dest=$_SESSION['erppath']."crm/dokumente/".$_SESSION["dbname"]."/".$pfad."/".$this->name;
		$this->logvar();
		if (chkdir($pfad)) {
			//Zielpfad vorhanden
			//if (! copy($_SESSION['erppath'].'crm/tmp/'.$file["Datei"]["tmp_name"],$dest)) {
            ...
            //unlink($_SESSION['erppath'].'crm/tmp/'.$file["Datei"]["tmp_name"]);
            @unlink($file["Datei"]["tmp_name"]);
            //chmod($dest,decoct($_SESSION['dir_mode'])); 
            $tmpdata = getUserEmployee(array('dir_mode','dir_group'));
            if ( $tmpdata['dir_group'] ) @chgrp($dest,$tmpdata['dir_group']); 
            if ( $tmpdata['dir_mode']  ) @chmod($dest,octdec($tmpdata['dir_mode'])); 
		} else {
...
*/
	function deleteDocument($p="") {
		...
		$dest=$_SESSION['erppath']."crm/dokumente/".$_SESSION["dbname"].$this->pfad."/".$this->name;
		$rc=unlink($dest);
		if (!$rc) {
