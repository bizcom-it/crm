...
function getTax() {
    ...
    $sql .= "TC.income_accno_id,TC.expense_accno_id ";
    ...
    if ($rs) foreach ($rs as $row) {
        ...
        if (!isset($TAX[$nr])) {
            ...
            $TAX[$nr]["taxid"] =      $row['taxid'];
            $TAX[$nr]["income"] =     $row['income_accno_id'];
            $TAX[$nr]["expense"] =    $row['expense_accno_id'];
        }
    }
    return $TAX;
}
...
function getArtikel($data) {
    $no = array('ok','preise','addtax','pglist','prozent','pm','order','vorlage','shop','image');
    $where = '';
    $cvar = array();
    $cvarjoin = '';
    ...
    if ($data) {
        foreach ($data as $key=>$val) {
           ...
           if (preg_match('/vc_cvar_([a-z]+)_(.+)/',$key,$hit)) {
               if ($val != '') $cvar[] = "(CVC.name='".$hit[2]."' and CV.".$hit[1]."_value='$val' and CV.trans_id=P.id)";
           }
           $val = strtr($val,'*?','%_');
           if ($key == 'partnumber' and strpos($val,',')) {
               $pnr = split(',',$val);
               foreach ($pnr as $nr) $pnumber[] = "'".strtoupper($nr)."'";
               $tmp[] = ' upper(partnumber) in ('.implode(',',$pnumber).') ';
               continue;
           }
    ...
    if ( isset($data['shop']) and $data['shop'] == '1' ) $where .= " and shop = 't' ";
    if ( isset($data['image']) and $data['image'] == '1' ) $where .= " and image != '' ";
    if ($data['preise']>2) { 
	$prices = ',PR.price '; 
...
function prepTex($tex='katalog',$upload=false) {
    ...
    $postline = false;
    $artline = false;
    if ( substr($tex,-4) == '.tex' ) $tex = substr($tex,0,-4);
    if ($upload) {
...
function getLager($data) {
   ...
   $order = 'p.';
   if ( isset($data['wg']) and $data['wg'] == '1') $order .= 'partsgroup_id,p.';
   $order .= $data['sort'];
   ...
   if ( isset($data['datum']) and  $data['datum']!= '') {
   ...
   $artikel = $GLOBALS['db']->getAll($sql); //,DB_FETCHMODE_ASSOC);
   return $artikel;
}
function getPartsgroup() {
   $sql = "SELECT * FROM partsgroup order by partsgroup";
   $pg = $GLOBALS['db']->getAll($sql); //,DB_FETCHMODE_ASSOC);
   return $pg;
}
function closeinventur($art,$name) {
    ...
    putenv('openin_any=p');
    //TEXINPUTS=''
    $rc = @exec("cd tmp; pdflatex -interaction=nonstopmode $art.tex; cd ..",$out,$ret);
    $rc = @exec("cd tmp; pdflatex -interaction=nonstopmode $art.tex; cd ..",$out,$ret);
    putenv('HOME='.$home);
...
function getPartBin($pg,$partnumber,$obsolete,$bin) {
    if ($pg == '' and $partnumber == '') {
	    $where = "(partsgroup_id is NULL or partsgroup_id = 0) ";
    } else if ( $pg != '' and $partnumber == '' ) {
       $where = "partsgroup_id = $pg ";
    } else {
        $partnumber = strtr($partnumber,'*?','%_'); 
        if ( strpos($partnumber,',') > 0 ) {
            $pnr = split(',',$partnumber);
            foreach ($pnr as $nr) $pnumber[] = "'".strtoupper($nr)."'";
            $where = ' upper(partnumber) in ('.implode(',',$pnumber).') ';
        } else {
            $where = "partnumber ilike '$partnumber' ";
        }
       if ($pg != '') $where .= " AND partsgroup_id = $pg";
    }
    ...
    $sql .= 'FROM parts p LEFT JOIN inventory i  ON i.parts_id  = p.id  LEFT JOIN bin   b ON i.bin_id  = b.id WHERE '.$where;
    ...
    $sql .= 'p.id AS parts_id, null, 0 as qty, p.unit AS partunit, p.onhand FROM parts p WHERE '.$where;
    if ( $obsolete != '' ) $sql .= " AND obsolete ='$obsolete' ";
