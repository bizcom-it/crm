...
function getonecontent( $id, $edit, $tiny ) {
    $data = getWContent( $id );
    if ( $edit == 1 ) {
    ...
    if ( isset($data[$HauptGrp]) ) {
        ...
        if (count($result) > 0) {
            ...
            while($thread[$HauptGrp]=array_shift($result)) {
                ...
                $menu.= "<li><a href='#' id='C".$thread[$HauptGrp]["id"]."' name='$kdh' class='sub".$HauptGrp."' onClick='toggleMenu(".$HauptGrp.','.$thread[$HauptGrp]["id"].")'>";
...
switch ($_GET['task']) {
    ...
    case 'filesearch'        : filesearch( );
                               break;
    case 'edit'              : getonecontent( $_GET['id'], $_GET['edit'], $_GET['tiny'] );
                               break;
    case 'neu'               : neucontent( $_GET['tiny'] );
                               break;
