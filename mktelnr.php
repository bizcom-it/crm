<?php
	...
	$rs=$GLOBALS['db']->getAll($sql1,DB_FETCHMODE_ASSOC);
	foreach($rs as $eintrag) {
		mkTelNummer($eintrag["id"],"C",array($eintrag["phone"]));
	}
	$rs=$GLOBALS['db']->getAll($sql3,DB_FETCHMODE_ASSOC);
	foreach($rs as $eintrag) {
		mkTelNummer($eintrag["id"],"V",array($eintrag["phone"]));
	}
	$rs=$GLOBALS['db']->getAll($sql4,DB_FETCHMODE_ASSOC);
	foreach($rs as $eintrag) {
		mkTelNummer($eintrag["id"],"E",array($eintrag["phone"]));
	}
	$rs1=$GLOBALS['db']->getAll($sql5,DB_FETCHMODE_ASSOC);
	$rs2=$GLOBALS['db']->getAll($sql6,DB_FETCHMODE_ASSOC);
	$rs3=$GLOBALS['db']->getAll($sql7,DB_FETCHMODE_ASSOC);
	$rs4=$GLOBALS['db']->getAll($sql8,DB_FETCHMODE_ASSOC);
	$rs5=$GLOBALS['db']->getAll($sql9,DB_FETCHMODE_ASSOC);
	$rs6=$GLOBALS['db']->getAll($sql10,DB_FETCHMODE_ASSOC);
