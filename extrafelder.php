...
function saveFelder($data) {
	...
	foreach ($data as $key=>$val) {
		...
		$val=trim($val);
		$rc=$GLOBALS['db']->insert('extra_felder',array('tab','owner','fkey','fval'),array($tab,$owner,$key,$val));
		if (!$rc) { $GLOBALS['db']->query("ROLLBACK"); return false; };
	}
...
$t->set_var(array(
    ...
    'visiblesichern' => $visible,
    'visiblesuchen'  => $hidden,
    'POPUP'          => '',
    'SUCHE'          => 'no',
    'ZIEL'           => 'extrafelder.php'
));
getFelder($owner,$t);
